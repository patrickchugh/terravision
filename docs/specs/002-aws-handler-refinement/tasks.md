...
## Validation Checklist (MANDATORY Before Declaring Phase Complete)
...
### 8. Visual Diagram Validation (Optional but Recommended)

**Purpose**: Verify diagram makes visual sense

```bash
# Generate actual diagram
poetry run python terravision.py draw \
  --source tests/fixtures/aws_terraform/<pattern> \
  --outfile /tmp/<pattern>.png
```
...
- [ ] No visual clutter (unnecessary intermediaries, duplicates)
