...
def _validate_source(source: list):
    """Validate that source entries are not individual .tf files.

    Args:
        source: List of source paths (directories or URLs)

    Raises:
        SystemExit: If any source is an individual .tf file
    """
    for src in source:
        if src.endswith(".tf"):
            click.echo(
                click.style(
                    f"\nERROR: You have passed a .tf file as source: {src}\n"
                    "Please pass a folder containing .tf files or a git URL.\n",
                    fg="red",
                    bold=True,
                )
            )
...
def draw(
    ...
    if debug:
        # When debug is True, keep default exception hook for full tracebacks
        pass
    else:
        # When debug is False, use simplified exception hook
    ...
    try:
        ...
        # Handle custom exceptions with appropriate styling
        if isinstance(e, MissingResourceError):
            ...
            if e.context:
                click.echo(click.style(f"Context: {e.context}", fg="yellow"))
            sys.exit(1)
        elif isinstance(e, ProviderDetectionError):
            click.echo(
                click.style(
            ...
            if e.context:
                click.echo(click.style(f"Context: {e.context}", fg="yellow"))
            sys.exit(1)
        elif isinstance(e, MetadataInconsistencyError):
            click.echo(
                click.style(
                    f"\nWARNING: Metadata validation issue\n{e.message}",
                    fg="yellow",
                    bold=True,
                )
            )
            if e.context:
                click.echo(click.style(f"Context: {e.context}", fg="yellow"))
        elif isinstance(e, TerraformParsingError):
            click.echo(
                click.style(
            ...
            if e.context:
                click.echo(click.style(f"Context: {e.context}", fg="yellow"))
            sys.exit(1)
        elif isinstance(e, TerraVisionError):
            # Generic TerraVision error
            click.echo(
                click.style(
                    f"\nINFO: {e.message}",
                    fg="yellow",
            ...
            # Re-raise non-TerraVision exceptions
            raise
