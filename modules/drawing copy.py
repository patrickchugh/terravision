...
def connect_up(origin: Node, destination: Node, metadata: dict):
    ...
    consolidated_dest_prefix = [
        k for k in consolidated_nodes if dest_resource.startswith(k)
    ]
    consolidated_origin_prefix = [
        k for k in consolidated_nodes if origin_resource.startswith(k)
    ]
    if consolidated_origin_prefix:
        candidate_resources = helpers.list_of_dictkeys_containing(
            metadata, consolidated_origin_prefix[0]
        )
    ...
    if edge_labels_list:
        for labeldict in edge_labels_list:
            ...
            if key == dest_resource or (
                consolidated_dest_prefix and key.startswith(consolidated_dest_prefix[0])
            ):
                label = labeldict[key]
                break
...
def handle_special_resources(tfdata: dict, graphdict: dict, resource: str):
    ...
    if lb_found:
        ...
        for nodename in lb_nodes:
            ...
            if not connected_nodes.get(
                origin_node._id
            ) or not dest_node._id in connected_nodes.get(origin_node._id):
    ...
    if resource.startswith("aws_nat_gateway."):
        igw_found = helpers.list_of_dictkeys_containing(
            graphdict, "aws_internet_gateway"
        )
...
def draw_vpc_subnets(tfdata, graphdict, resource, connections_list):
    ...
    def draw_subnets():
        ...
        for i in range(count):
            ...
            subnet_identifier = helpers.cleanup(
                subnet_identifier
                + " "
                + tfdata["meta_data"][connected_item]["cidr_block"]
            )
            if (
                "[" in subnet_identifier
                and "count.index" in subnet_identifier
                or "each.value" in subnet_identifier
            ):
            ...
            for item in graphdict[connected_item]:
                if (
                    item.startswith("aws_nat_gateway")
                    and "public" not in subnet_identifier
                ):
            ...
            with SubnetGroup(subnet_identifier):
                for subnet_item in graphdict[connected_item]:
                    variant = check_variant(
                        subnet_item, tfdata["meta_data"][subnet_item]
                    )
                ...
                if not has_elements:
                    ...
                    blank_node = getattr(sys.modules[__name__], "Node")(
                        **invisible_style
                    )
    ...
    for i in range(count):
        with VPCgroup(
            resource_label
            + " VPC "
            + helpers.cleanup(tfdata["meta_data"][resource]["cidr_block"])
        ) as VPC_Group:
            with AvailabilityZone():
                ...
                for connected_item in connections_list:
                    ...
                    if connected_service_name == "aws_subnet":
                        ...
                        variant = check_variant(
                            connected_item, tfdata["meta_data"][connected_item]
                        )
                        node = getattr(
                            sys.modules[__name__], connected_service_name + variant
                        )(label=connected_label, tf_resource_name=connected_item)
                        tfdata["meta_data"][connected_item]["node"] = node
    return VPC_Group


def draw_child_nodes(
    parentnode: Node, connections_list: list, tfdata: dict, graphdict: dict
):
    ...
    for connected_item in connections_list:
        ...
        if tfdata["meta_data"][connected_item].get("label"):
            connected_label = helpers.pretty_name(
                tfdata["meta_data"][connected_item]["label"]
            )
        ...
        if connected_service_name in avl_classes:
            if "node" in tfdata["meta_data"][connected_item]:
                ...
                if (
                    connected_service_name == "aws_cloudfront_distribution"
                    and "lambda_function_association"
                    in str(tfdata["meta_data"][connected_item])
                ):
                    with GenericGroup("Lambda@Edge"):
                        draw_child_nodes(
                            childnode, graphdict[connected_item], tfdata, graphdict
                        )
    ...
    with group:
        ...
        for resource, connections_list in graphdict.items():
            ...
            if tfdata["meta_data"][resource].get("label"):
                resource_label = helpers.pretty_name(
                    tfdata["meta_data"][resource]["label"]
                )
            if (
                resource_service_name in avl_classes
                and resource_service_name not in group_nodes
            ):
                # Just draw one node if there are no connections
                if len(connections_list) == 0:
                    if not "node" in tfdata["meta_data"][resource]:
                        parentnode = getattr(
                            sys.modules[__name__], resource_service_name + variant
                        )(label=resource_label, tf_resource_name=resource)
...
def draw_if_consolidated(
    ...
    for check_consolidate in consolidated_nodes.keys():
        ...
        already_drawn_node = (
            ...
            and tfdata["meta_data"].get(check_resource_name).get("node") != None
        )
        undrawn_consolidated_service = (
            nodetype.startswith(check_consolidate) and not already_drawn_node
        )
        ...
        shared_service = consolidated_nodes[check_consolidate][
            "vpc"
        ] == False and not consolidated_nodes[check_consolidate].get("edge_service")
        if undrawn_consolidated_service:
            ...
            if consolidated_nodes[check_consolidate]["vpc"] == True and not vpc_exists:
                click.echo(
                    "WARNING: VPC expected in this architecture but has not been defined in the Terraform source specified"
                )
                ok_to_draw = True
            ...
            if ok_to_draw:
                ...
                if nodetype.startswith(check_consolidate):
                    if tfdata["meta_data"][nodetype].get("label"):
                        label = helpers.pretty_name(
                            tfdata["meta_data"][nodetype]["label"]
                        )
                MyClass = getattr(
                    importlib.import_module(
                        consolidated_nodes[check_consolidate]["import_location"]
                    ),
                    check_consolidate,
                )
        ...
        if resource_service_name in avl_classes:
            ...
            for check_consolidate in consolidated_nodes.keys():
                if node.startswith(check_consolidate):
                    grouped_name = consolidated_nodes[check_consolidate][
                        "resource_name"
                    ]
                    if ".*" in grouped_name:
                        for actual_name in graphdict:
...
def render_diagram(
    ...
    if tfdata["annotations"]:
        ...
        tfdata["meta_data"] = modify_metadata(
            tfdata["annotations"], graphdict, tfdata["meta_data"]
        )
    ...
    for node in tfdata["node_list"]:
        draw_if_consolidated(
            node, tfdata, graphdict, SharedServicesCluster, False, vpc_exists
        )
