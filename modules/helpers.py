        ...
        for local_var in local_found_list:
            ...
            for file, localvalues in all_locals.items():
                if local_var in localvalues:
                    ...
                    eval_string = eval_string.replace(
                        "local." + local_var, resolved_local
                    )
                    eval_string = fix_lists(eval_string)
    return eval_string
...
def check_relationship(
    listitem: str, plist: list, nodes: list, replacements: list, hidden: list
) -> list:
    connection_list = []
        ...
        for matched_resource in matching:
            if (
                ...
                if reverse:
                    ...
                    click.echo(f"   {matched_resource} --> {resource_associated_with}")
                elif (
                    not "aws_acm" in resource_associated_with
                ):  # Exception Ignore outgoing connections from ACM certificates and resources mentioned in depends on
...
# Function to generate a dict structure listing all the nodes and a list of connected nodes
def make_graph_dict(
    nodelist: list,
    all_resources: dict,
    all_locals: dict,
